...
let
  ...
  modules =
    {
      machineName,
      machineModule ? network.${machineName},
      nodes,
      check,
    }:
    [
  ...
  networkMachines = removeAttrs network [
    "network"
    "defaults"
    "resources"
    "require"
...
rec {
  ...
  uncheckedNodes = builtins.mapAttrs (
        ...
        in
        {
          ...
          nixosRelease =
            v.config.system.nixos.release
              or (nwLib.removeSuffix v.config.system.nixos.version.suffix v.config.system.nixos.version);
  ...
  machines =
    ...
    let
      ...
      nodes' = nwLib.filterAttrs (n: v: builtins.elem n fileArgs.Names) nodes;
    in
    nwRunCommand "morph" { preferLocalBuild = true; } (
      if buildTargets == null then
        ''
          mkdir -p $out
          ${toString (
            nwLib.mapAttrsToList (nodeName: nodeDef: ''
        ...
        ''
          ...
          ${toString (
            nwLib.mapAttrsToList (nodeName: nodeDef: ''
              ...
              ${toString (
                nwLib.mapAttrsToList (buildName: buildFn: ''
